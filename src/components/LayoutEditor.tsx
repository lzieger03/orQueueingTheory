    ...
    >
      ...
      {isSimulating && queuePositions.map((pos, index) => (
        <div
          ...
          style={{ left: pos.x - station.position.x, top: pos.y - station.position.y }}
        />
      ))}
...
export function LayoutEditor({ 
  ...
  return (
    <div className="relative w-full h-full bg-gradient-to-br from-purple-900/20 to-blue-900/20 backdrop-blur-sm rounded-xl overflow-hidden border border-white/20 shadow-lg">
      ...
      >
        {/* Store Entrance */}
        <div className="absolute top-0 left-1/2 transform -translate-x-1/2 w-20 h-6 bg-gradient-to-r from-green-400 to-emerald-400 rounded-b-xl shadow-lg border border-green-300/50">
        ...
        {isSimulating && Array.isArray(customers) && customers.map(customer => {
          ...
          return (
            <div
              key={customer.id}
              className="customer-icon absolute animate-float"
